# Manual smoke test (REPLAY): Mages Celestial_reversal — the replay must mirror the
# engine's day/night fix:
#   * the INSTANT effect (day/night fixed to the player's choice) is applied at play
#     time (turn 1) in the replay's local day/night tracking
#   * the day/night does NOT flip after the fix (turn 2 keeps the fixed value)
#   * the Celestial_reversal card is a no-op on the trip chain (advancing 0)
#   * no "unknown card" warnings (the replay's _card_row synthesizes a support row)
# Run from the project root:  uv run python tests/_mages_celestial_replay.py
...
CELESTIAL = ge.MAGE_CELASTIAL_REVERSAL
...
# --- build the game: A has Celestial_reversal + 5 distinct; B has 6 distinct ----
# (20-card decks, 20 distinct main cards each -> no duplicate-deck warning)
a_fill = CARDS[0:19]          # 19 distinct
b_fill = CARDS[6:26] if len(CARDS) >= 26 else CARDS[0:20]
# make sure A and B use disjoint sets where possible to keep the hands clean
a_deck = [CELESTIAL] + CARDS[0:19]
b_deck = CARDS[6:26]
...
# A's hand: Celestial_reversal + 5 cards; B's hand: 6 cards
ga.hand = [CELESTIAL, CARDS[0], CARDS[1], CARDS[2], CARDS[3], CARDS[4]]
ga.deck = a_deck[6:]
        ...
        if actor == 'A':
            if not a_played and a_card:
                gs, r = send(gid, 'A', {'cards': [a_card], 'to': 'stopover_4', 'mode': 'move', 'pendings': [],
                                       **({'day_night': 'night'} if a_card == CELESTIAL else {})})
...
try:
    ...
    # --- 2 complete turns (Celestial_reversal on turn 1) ---------------------
    # turn 1: PLAY phase (turn 1 skips the mana phase - init covered it)
    gs = turn(gs, CELESTIAL, b_deck[3])   # A fixes night, B plays a card
    print(f"  after turn 1: state={gs.state!r} turn={gs.turn} day_night={gs.day_night} fixed={gs.day_night_fixed}")
    # turn 2: MANA phase (both pass) -> PLAY phase
    gs = mana_pass(gs)
    print(f"  after turn 2 mana: state={gs.state!r} turn={gs.turn}")
    gs = turn(gs, CARDS[3], b_deck[4])
    print(f"  after turn 2: state={gs.state!r} turn={gs.turn} day_night={gs.day_night} fixed={gs.day_night_fixed}")

    check('2 turns completed (now turn 3)', gs.turn == 3, f"(turn={gs.turn})")
    check('stored day/night is still night (fixed)', gs.day_night == 'night', f"(day_night={gs.day_night})")
    ...
    dn = [t.get('day_night') for t in res['turns']]
    print(f"  replay per-turn day/night: {dn}")
    check('replayed 2 turns', len(res['turns']) >= 2, f"(turns={len(res['turns'])})")
    check('replay turn 1 day/night = night (the fix)', dn[0] == 'night', f"(dn={dn})")
    check('replay turn 2 day/night = night (NO flip)', len(dn) < 2 or dn[1] == 'night', f"(dn={dn})")
    check('no "unknown card" warnings for Celestial_reversal',
          not any('Celestial_reversal' in w and 'unknown' in w for w in res['warnings']),
          f"(warnings={res['warnings']})")
    # the replay should have recorded the celestial_reversal event
    evs = [e for t in res['turns'] for e in t.get('events', []) if e.get('type') == 'celestial_reversal']
    check('replay recorded the celestial_reversal event', len(evs) == 1, f"(events={evs})")

    print(f"\nALL {ok} REPLAY CHECKS PASSED - the replay mirrors the Celestial_reversal day/night fix")
finally:

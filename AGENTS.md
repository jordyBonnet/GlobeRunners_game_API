...
A **main deck** is **20 cards, single main faction, single-copy** (no duplicates), with **at most 5 cards per condition family** and **at most 5 cards per effect**. Numeric variants of a condition are ONE family: `temp_inf_6`≡`temp_inf_11`, `temp_sup_9`≡`temp_sup_15`, `dist_ahead_sup_1`≡`dist_ahead_sup_3`, `dist_behind_sup_1`≡`dist_behind_sup_3` (every other condition is its own family). `face_point_left` and `face_point_right` (condition OR effect) are **not yet implemented in the online game — decks containing them are refused**. The **support deck** is **2 copies of each of the 5 cards of ONE support faction** (10 cards).

- **`deck_rules.py`** (project root) is the **single source of truth**: `check_main_deck` / `check_support_deck` / `check_deck` return a list of human-readable violations (empty = valid). `API.py`'s `check_deck` (called on `/create_game` + `/join_game`) delegates to it — an invalid deck is a **400 with every violation listed**.
- **Frontend mirror** (`game_ui/static/cards.mjs` → `checkMainDeck`): the setup page shows a live ✓/✗ issues panel under the deck preview (`renderDeckIssues`, `setup.mjs`) and **blocks launch** while any rule is violated; the CSV import no longer rejects duplicates silently — duplicates surface in the issues panel.
- **`game_ui/ai_deck.py`** — the **AI deck builder**: `build_ai_deck()` = random main faction + random support faction + a 20-card main deck that follows the support-faction strategy rules (Engineers: 5×`drop_on_board`, no `pending`/extreme-temp/`draw`; Mages: 2–4× extreme temp, 4× day/night, 4× own-biome, no drop/pending; Doctors: 5×`pending`, no drop/extreme-temp) + the general rules (2×`wrecking_ball`, 2×`jump`, 2–4×`draw` except Engineers, 2–4× the faction special effect) + the per-faction rules (Mummies: some `*_oppo` state cards; Dwarves: some `block`). Priority when infeasible: support > general > per-faction (the general 2–4 ranges are relaxed first, still in range; only Mummies+Mages truly cannot fit the per-faction rule — the minimums total 22 > 20). Rarity-weighted randomness (rare ×1, other ×3). `ai_driver.random_ai_deck` delegates to it.
- **Tests**: `tests/_deck_rules.py` (rule matrix incl. family merging + banned values) and `tests/_ai_deck.py` (all 18 faction×support combinations × 25 builds — 6480 checks).
...
- **AI**: since A.1 (`player_ai/AI_improvement_list.md`, ✅ 2026-09-21) `PlayerAI` **does** play support cards, so the robot CAN create pending/dwelling placeholders: `ai_decide` therefore computes every move/defend/support-play stopover with `played_before = me.play_count` (the engine's own counter — it counts moves+defends AND placeholder placements; an action_chain-derived sum under-counts). The engine remains the single source truth (`player_play` overwrites the client's `to`). `put_mana` no longer burns support cards first (keep-value model). Since I.1 (2026-09-24) the **mana phase is a decision, not an automatic**: the robot **passes** when its available mana already covers the **total cost of its whole hand** (`_hand_total_cost`) and only places a card in mana while the pool is still short of that total (a sacrificed card is never played again). Since A.2 (same date) the robot also **attaches** pendings to move plays (`pendings: [name]` via `_choose_attachment` — epo/mercurochrome only, never when the effect cannot fire; see *A.2* in [AI_improvement_list](player_ai/AI_improvement_list.md)). Since §C–§G (2026-09-21/22) it also scores plays with a resolution-exact movement model + win detection + pass discipline (§C), reacts to declared threats with value-based multi-card defends instead of a coin flip (`choose_defend`, §D), and places reactive effects by what they face — `swap_with` decoys, `effect_canceled`/grappling/copy targeting (§E). The robot is **fair-play by default**: it never reads the opponent's hidden hand/deck/mana contents (only public counts); `run_ai_loop(..., hard_mode=True)` is an explicit opt-in for perfect information that only removes false-positive threat guesses, never adds threats. Status of every item: [player_ai/AI_improvement_list.md](player_ai/AI_improvement_list.md) (§A–§G + §I done, §H open). See *A.1*.
...
- `deck_rules.py` — deck-rule validation (single source of truth; see *Deck rules (validation)* in this file) — `tests/_deck_rules.py`
- `game_ui/ai_deck.py` — AI deck builder (support-faction strategy rules + deck rules) — `tests/_ai_deck.py`

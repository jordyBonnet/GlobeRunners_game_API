...
uv run python tests/_game_over_hook.py # game-over hook: fires EXACTLY ONCE on the transition to "game over" (win/mid-chain/deadlock) + not again on stray post-game-over messages; with a token it also verifies the real upload of the finished game to the HF dataset (jordyBonnet/globerunners-games, games/<game_id>.json) and cleans up (deletes its own games + dataset file)

...
8. **State application** — `applyState(st)` (single funnel for WS replies **and** poll responses: resets cell-select, stores state, `renderAll()`, shows server message, endgame detection) → `renderAll()` (topbar, phase label + action buttons, then the zone/board/log renderers below).
...
- **Full re-render per state**: `applyState → renderAll` rewrites the zones, board rows and slots from scratch (the DOM is treated as a function of `game.state`). The two exceptions are deliberately incremental: the **log** (`game.logTurnsRendered`) and the **selection/cell-select** transient UI.

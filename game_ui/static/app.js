...
function applyState(st, fromPolling) {
  ...
  // stopover→discard, …) — see the "card movement animation" section below
...
/* ---------------- card movement animation (deck→hand, hand→stopover, stopover→discard, …) ----------------
...
const FLY_MS = 520;                          // one card flight duration
const FLY_STAGGER_MS = 110;                  // delay between consecutive flights
...
function animateZoneTransitions(oldSt, newSt, snap) {
  const moves = [];
...
/* ---------------- trip-chain resolution animation ----------------
